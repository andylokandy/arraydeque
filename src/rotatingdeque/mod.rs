...
/// `RotatingDeque` is a rotating, fixed capacity ring buffer.
///
/// It can be stored directly on the stack if needed.
///
/// The "default" usage of this type as a queue is to use `push_back` to add to
/// the queue, and `pop_front` to remove from the queue. `extend` and `append`
/// push onto the back in this manner, and iterating over `RotatingDeque` goes front
/// to back.
///
/// If full, pushing an additional object onto the back (front) of the queue
/// will cause the first (last) element to be overwritten.
///
/// # Capacity
///
...
pub struct RotatingDeque<A: Array> {
    xs: NoDrop<A>,
    head: A::Index,
    tail: A::Index,
}

/// `RotatingDeque` iterator
...
/// `RotatingDeque` mutable iterator
#[must_use = "iterator adaptors are lazy and do nothing unless consumed"]
...
/// A by-value `RotatingDeque` iterator
#[must_use = "iterator adaptors are lazy and do nothing unless consumed"]
pub struct IntoIter<A: Array> {
    inner: RotatingDeque<A>,
}

/// A draining `RotatingDeque` iterator
          ...
          A::Item: 'a
{
    ...
    after_head: usize,
    iter: Iter<'a, A::Item>,
    deque: *mut RotatingDeque<A>,
}

impl<A: Array> RotatingDeque<A> {
    /// Creates an empty `RotatingDeque`.
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// let vector: RotatingDeque<[usize; 3]> = RotatingDeque::new();
    /// ```
    #[inline]
    pub fn new() -> RotatingDeque<A> {
        unsafe {
            RotatingDeque {
    ...
    /// Retrieves an element in the `RotatingDeque` by index.
    ///
    ...
    /// use arraydeque::RotatingDeque;
    ///
    /// let mut buf: RotatingDeque<[_; 4]> = RotatingDeque::new();
    /// buf.push_back(3);
    /// buf.push_back(4);
    /// buf.push_back(5);
    ...
    /// Retrieves an element in the `RotatingDeque` mutably by index.
    ///
    /// Element at index 0 is the front of the queue.
    ///
    /// # Examples
    ...
    /// use arraydeque::RotatingDeque;
    ///
    /// let mut buf: RotatingDeque<[usize; 4]> = RotatingDeque::new();
    ...
    /// use arraydeque::RotatingDeque;
    ///
    /// let mut vector: RotatingDeque<[_; 6]> = RotatingDeque::new();
    ///
    /// vector.push_back(0);
    /// vector.push_back(1);
    /// vector.push_back(2);
    ...
    /// use arraydeque::RotatingDeque;
    ///
    /// let mut vector: RotatingDeque<[_; 5]> = RotatingDeque::new();
    ///
    /// vector.push_back(0);
    /// vector.push_back(1);
    ///
    ...
    /// Returns the number of elements in the `RotatingDeque`.
    ///
    /// # Examples
    ///
    /// ```
    /// use arraydeque::RotatingDeque;
    ///
    /// let mut v: RotatingDeque<[_; 4]> = RotatingDeque::new();
    /// assert_eq!(v.len(), 0);
    /// v.push_back(1);
    /// assert_eq!(v.len(), 1);
    /// ```
    ...
    /// `RotatingDeque` and yields the removed items.
    ///
    ...
    /// use arraydeque::RotatingDeque;
    ///
    /// let mut v: RotatingDeque<[_; 4]> = vec![1, 2, 3].into_iter().collect();
    /// assert_eq!(vec![3].into_iter().collect::<RotatingDeque<[_; 4]>>(), v.drain(2..).collect());
    /// assert_eq!(vec![1, 2].into_iter().collect::<RotatingDeque<[_; 4]>>(), v);
    ///
    ...
    /// Returns `true` if the `RotatingDeque` contains an element equal to the
    /// given value.
    ///
    /// # Examples
    ///
    ...
    /// use arraydeque::RotatingDeque;
    ///
    /// let mut vector: RotatingDeque<[_; 3]> = RotatingDeque::new();
    ///
    /// vector.push_back(0);
    /// vector.push_back(1);
    ///
    ...
    /// use arraydeque::RotatingDeque;
    ///
    /// let mut d: RotatingDeque<[_; 3]> = RotatingDeque::new();
    ...
    /// Inserts an element first in the sequence, overwriting the head if full.
    ///
    ...
    /// Appends an element to the back of a buffer, overwriting the tail if full.
    ///
    /// Return `None` if the push is possible without overwriting an element, otherwise return
    /// `Some(overwritten)` (which happens when the underlying storage is full).
    ///
    ...
    pub fn push_back(&mut self, element: A::Item) -> Option<A::Item> {
        if self.is_full() {
            unsafe {
                ...
                self.set_tail(Self::wrap_add(tail,1));
                Some(self.buffer_write(head, element));
                Some(self.buffer_read(tail))
            }
        } else {
            unsafe {
    ...
    /// Removes an element from anywhere in the `RotatingDeque` and returns it, replacing it with
    /// the last element.
    ///
    ...
    /// Removes an element from anywhere in the `RotatingDeque` and returns it,
    ...
    /// Inserts an element at `index` within the `RotatingDeque`. Whichever
    ...
    /// # Panics
    ///
    /// Panics if `index` is greater than `RotatingDeque`'s length
    ///
    /// # Examples
    /// ```
    /// use arraydeque::RotatingDeque;
    ...
    /// Removes and returns the element at `index` from the `RotatingDeque`.
    ...
    /// use arraydeque::RotatingDeque;
    ///
    /// let mut buf: RotatingDeque<[_; 4]> = vec![1,2,3].into_iter().collect();
    ...
    /// use arraydeque::RotatingDeque;
    ///
    /// let mut buf: RotatingDeque<[_; 8]> = vec![1, 2, 3].into_iter().collect();
    /// // buf2: RotatingDeque<[_; 8]> can be infer
    ...
    /// use arraydeque::RotatingDeque;
    ///
    /// let mut buf: RotatingDeque<[_; 5]> = RotatingDeque::new();
    /// buf.extend(1..5);
    /// buf.retain(|&x| x%2 == 0);
    ///
...
mod tests {
    ...
    #[test]
    fn test_index() {
        let mut tester: RotatingDeque<[_; 4]> = RotatingDeque::new();
        tester.push_back(1);
        tester.push_back(2);
        tester.push_back(3);
    ...
    fn test_into_iter() {
        ...
        {
            let mut tester: RotatingDeque<[NoCopy<u8>; 3]> = RotatingDeque::new();
    ...
    fn test_drain() {
        const CAP: usize = 7;
        let mut tester: RotatingDeque<[_; CAP + 1]> = RotatingDeque::new();
    ...
    fn test_drop() {
        ...
        {
            let mut tester = RotatingDeque::<[Bump; 128]>::new();
        ...
        flag.set(0);
        {
            let mut tester = RotatingDeque::<[_; 4]>::new();
    ...
    fn test_as_slice() {
        const CAP: usize = 10;
        let mut tester = RotatingDeque::<[_; CAP]>::new();
    ...
    fn test_partial_equal() {
        const CAP: usize = 10;
        let mut tester = RotatingDeque::<[f64; CAP]>::new();
        ...
        for len in 0..CAP - 1 {
            for padding in 0..CAP {
                ...
                let mut expected = RotatingDeque::<[f64; CAP]>::new();
                for x in 0..len {
    ...
    #[test]
    fn test_fmt() {
        let mut tester = RotatingDeque::<[_; 5]>::new();
        tester.extend(0..4);
    ...
    fn test_from_iterator() {
        let tester: RotatingDeque<[_; 5]>;
        let mut expected = RotatingDeque::<[_; 5]>::new();
    ...
    #[test]
    fn test_extend() {
        let mut tester: RotatingDeque<[usize; 10]>;
        tester = vec![0, 1, 2, 3].into_iter().collect();
        tester.extend(vec![4, 5, 6, 7]);
        let expected = (0..8).collect::<RotatingDeque<[usize; 10]>>();
        assert_eq!(tester, expected);
    }
    ...
    fn test_retain() {
        const CAP: usize = 10;
        let mut tester: RotatingDeque<[_; CAP]> = RotatingDeque::new();
        for padding in 0..CAP {
            unsafe {
                tester.set_tail(padding);
                tester.set_head(padding);
    ...
    #[test]
    fn test_append() {
        let mut a: RotatingDeque<[_; 16]> = vec![1, 2, 3].into_iter().collect();
        let mut b: RotatingDeque<[_; 16]> = vec![4, 5, 6].into_iter().collect();
    ...
    #[test]
    fn test_clone() {
        let tester: RotatingDeque<[_; 16]> = (0..16).into_iter().collect();
...
mod test_generic_array {
    ...
    #[test]
    fn test_simple() {
        let mut vec: RotatingDeque<GenericArray<i32, U41>> = RotatingDeque::new();
